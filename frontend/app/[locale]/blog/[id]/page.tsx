        ...
        <div className="container mx-auto px-4 py-12" style={{ maxWidth: 1200 }}>
          {loading ? (
            ...
            <article>
              ...
              <div className="mb-12">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-bold uppercase tracking-widest text-muted-foreground mb-4">
                  ...
                  <span>{calculateReadTime(post.content)} {t('minRead')}</span>
                  <span className="text-muted-foreground/30">•</span>
                  <span>{t('team')}</span>
                </div>
              ...
              <div className="mt-20 pt-12 border-t">
                <h2 className="text-2xl font-extrabold tracking-tighter uppercase mb-6">{t('nextSteps')}</h2>
                <p className="text-xl text-muted-foreground leading-relaxed mb-8">
                  {t('nextStepsDesc')}
                </p>
                <div className="flex flex-wrap gap-4">
                  <Link href="/">
                    <Button className="font-bold uppercase tracking-tight rounded-none h-12 px-8">{t('browseAgents')}</Button>
                  </Link>
                  <Link href="/sourcing">
                    <Button variant="outline" className="font-bold uppercase tracking-tight rounded-none h-12 px-8">{t('customRequest')}</Button>
                  </Link>
                </div>
              </div>
            </article>
            ...
            <div className="text-center py-20">
              ...
              <Link href="/blog">
                <Button variant="outline">{t('backToBlogBtn')}</Button>
              </Link>
            </div>
          )}
        </div>
        ...
        {!loading && post && relatedPosts.length > 0 && (
          <div className="bg-muted/30 py-24 border-t mt-20">
            <div className="container mx-auto px-4 max-w-6xl">
              <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground mb-16 text-center">{t('moreFromBlog')}</h2>
